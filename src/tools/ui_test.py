...
class CSVProcessorWorker(QThread):
    progress_updated = pyqtSignal(int, int, int)  # processed, total, success
    ...
    def __init__(self, csv_path: str, url_column: str, output_dir: str, 
                 ...
                 screenshot_type: str, start_row: int):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            def progress_callback(processed: int, total: int, success: int):
                self.progress_updated.emit(processed, total, success)
...
class ColorizedPandasModel(QAbstractTableModel):
    def __init__(self, df=pd.DataFrame(), parent=None):
        super().__init__(parent)
        self._df = df
        self.processed_rows = set()  # Track processed row indices
        self.success_rows = set()    # Track successful row indices
        self.failed_rows = set()     # Track failed row indices
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        # Color coding based on processing status
            ...
            if row in self.success_rows:
                return QBrush(QColor(200, 255, 200))  # Light green for success
            elif row in self.failed_rows:
                return QBrush(QColor(255, 200, 200))  # Light red for failure
            elif row in self.processed_rows:
                return QBrush(QColor(255, 255, 200))  # Light yellow for processing
    ...
    def update_processing_status(self, row_index, success):
        """Update the processing status of a row"""
        if success:
            self.success_rows.add(row_index)
        else:
            self.failed_rows.add(row_index)
...
class ScreenshotUI(QMainWindow):
    ...
    def select_all(self):
        ...
        if self.model:
            # Create a selection column if it doesn't exist
            if 'selected' not in self.model._df.columns:
                self.model._df['selected'] = True
            else:
                self.model._df['selected'] = True
            ...
            selected_count = len(self.model._df[self.model._df['selected'] == True])
            QMessageBox.information(self, "Sélection", f"{selected_count} éléments sélectionnés")
    ...
    def deselect_all(self):
        ...
        if self.model and 'selected' in self.model._df.columns:
            self.model._df['selected'] = False
            self.model.layoutChanged.emit()
            QMessageBox.information(self, "Désélection", "Tous les éléments désélectionnés")
            self.status_bar.showMessage("Tous les éléments désélectionnés")
        elif self.model:
            QMessageBox.information(self, "Info", "Aucun élément n'est sélectionné")
        else:
    ...
    def update_progress(self, processed, total, success):
